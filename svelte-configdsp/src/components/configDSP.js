...
function createIIRBlockXML(doc, form) {
    ...
    // handle the set - channel mapping
    while (input.name.includes("ChannelSet")) {
        var setChannelMapping = input.name.split(".");
        var numParentTags = setChannelMapping.length;
        var mappingString = setChannelMapping[numParentTags - 1];
        var channelIdxMapping = mappingString.split("_");
        channelIdx = channelIdxMapping[channelIdxMapping.length - 1];
        var setIdx = input.value;
        if (setIdx.indexOf('_') >= 0) {
            setIdx = setIdx.split('_')[1];
        }
        paramElement.setAttribute("name", "chan_" + channelIdx + "_set");
        paramElement.setAttribute("value", setIdx);
...
function createFIRBlockXML(doc, form)
{
    var blockElement = doc.createElement("block");
    var idElement = doc.createElement("id");
    var id = doc.createTextNode(form.id);
    ...
    for (var i = 2, ii = form.length; i < ii; i++) {
        ...
        if (input.name) {
            ...
            if (isDebugParameter(input.name)) {
                ...
                // we need to check if its coeffs or not.
                // coeffs are a csv list that must be split out into
                // individual params
                if (input.name.indexOf("coeffs") != -1) {
                    var coeffList = input.value.split(',');
                    for (var k = 0; k < coeffList.length; k++) {
                        var paramElement = doc.createElement("param");
                        paramElement.setAttribute("name", "coeff_" + k);
                        paramElement.setAttribute("value", coeffList[k]);
                        parentElement.appendChild(paramElement);
                    }
                }
                else {
...
function createControlBlockXML(doc, form)
{
    var blockElement = doc.createElement("controlBlock");
    var idElement = doc.createElement("id");
    var id = doc.createTextNode(form.id);
    var modeElement = doc.createElement("mode");
    // append nodes to parents
...
function onSubmitValues(e)
{
    ...
    var block;
    if(form.type == "IIRBlock" || form.type == "RampIIRBlock" || form.type == "XFadeIIRBlock" || form.type == "XFadeTwinIIR"){
        block = createIIRBlockXML(doc, this);
    }
    else if (form.type == "FIRBlock") {
        block = createFIRBlockXML(doc, this);
    }
    else if (form.type == "controlBlock") {
        block = createControlBlockXML(doc, this);
    }
    else {
...
// custom parameter handler for FIR Block
function getFIRParametersTable(blockId, blockType, parameters)
{
    // parameter table
    ...
    var numElements = parameters.childElementCount;
     for ( var k= 0; k < numElements;k++){
        if (topLevelElement.tagName == 'param') {
            ...
            table = populateParameter(table, blockId, topLevelElement, configurable);
        }
        else if (topLevelElement.tagName.indexOf('coeffs') == 0) {
            var tr = document.createElement('tr');

            var tdName = document.createElement('td');
            tdName.innerHTML = topLevelElement.tagName;
            var tdValue = document.createElement('td');
            var topCoeff = topLevelElement.firstElementChild;
            var numCoeffs = topLevelElement.childElementCount;
            var coeffs = [];
            for (var l = 0; l < numCoeffs; l++) {
                coeffs.push(topCoeff.getAttribute("value"));
                topCoeff = topCoeff.nextElementSibling;
            }

            var fieldId = blockId + "." + topLevelElement.tagName;
            add_children(tdValue, textfield(fieldId, coeffs.join(',')));
...
function getIIRParametersTable(blockId, blockType, parameters)
{
    ...
    var setOptions = new Array();
    for (var i =0 ; i < parameters.childNodes.length; i++) {
        var node = parameters.childNodes[i];

        if (node.nodeName == 'param') {
            var nodeName = node.getAttribute('name');
            if (nodeName.indexOf('coeffset') == 0) {
                // find all the coeff sets available
                var setIdx = nodeName.split('_')[2];
                var setName = node.getAttribute('value');
                if (setName.length > 0) {
                    setOptions.push(setName + "_" + setIdx);
                } else {
                    setOptions.push(setIdx)
                }
            } else if (nodeName.indexOf("active_iir") == 0) {
                var activeIIRTable = document.createElement('table');
                activeIIRTable.setAttribute("border", 1);
                activeIIRTable.setAttribute("cellpadding", 3);
                var trParam = document.createElement('tr');
                var tdName = document.createElement('td');
                var tdValue = document.createElement('td');
                tdName.innerHTML = "Active IIR";
                tdValue.innerHTML = node.getAttribute('value');
                add_children(trParam, tdName);
                add_children(trParam, tdValue);
                add_children(activeIIRTable, trParam);
                add_children(divTables, activeIIRTable);
            }
        }
    ...
    for (var i =0 ; i < parameters.childNodes.length; i++) {
        ...
        if (node.nodeName == 'param') {
            ...
            if (nodeName.indexOf('chan') == 0) {
                ...
                var channelID = nodeName.substring(0, nodeName.indexOf('set') - 1);
                var channelIndex = channelID.split('_')[1];
                var options = new Array();
                var max_options = setOptions.length;
                for (var j = 0; j < max_options; j++){
                    var option = document.createElement("option");
                    option.text = setOptions[j];
                    options[j] = option;
                 }
                var setIdx = parseInt(node.getAttribute('value'));
                var setSelect = dropdown(blockId + "_ChannelSet_" + channelIndex, options, setOptions[setIdx]);
                add_children(tdChannelMap, setSelect);
                tdChannel.innerHTML = channelID;
...
function getParentBlock(paramNode)
{
    ...
    if (parentNode.tagName == "block" ||
        parentNode.tagName == "controlBlock") {
        return parentNode;
    } else {
...
function addBlockToPage(block)
{
    ...
    for (var i = 0; i < block.childNodes.length; i++) {
        ...
        } else if (child.tagName == "controlBlock") {
            var subElem = addControlBlockToPage(child);
            add_children(table, table_entry("", subElem));
...
//TODO does the mode dropdown need to be part of the submitted form?
	    ...
	    debugDD = dropdown(blockId + ".debug",options,child.textContent);
            ...
            if(blockType == "IIRBlock" || blockType == "RampIIRBlock" || blockType == "XFadeIIRBlock" || blockType == "XFadeTwinIIR"){
               ...
               blockType == "MultiGain" ||
               blockType == "DRC" ||
               blockType == "SpeechEnhance" ||
               blockType == "Meter" ){
                parameters = getArrayParametersTable(blockId, blockType, child);
            }
            else if (blockType == "FIRBlock") {
               parameters = getFIRParametersTable(blockId, blockType, child);
            }
            else
    ...
    if (parameters != undefined){
        ...
        var export_cb = checkbox("checkboxExport_" + blockId, checkbox_state, exportCheckBoxChecked);
...
function addControlBlockToPage(block)
{
    // add this block to the page
    var elem = document.createElement('div');
    elem.innerHTML = "block: " + block.getAttribute("name") + "<br>";
    ...
    var blockId = undefined;
    var blockMode = undefined;
    var blockType = "controlBlock";
    var modeDD = undefined;
    var parameters = undefined;
    for (var i = 0; i < block.childNodes.length; i++) {
        var child = block.childNodes[i];
        if (child.tagName == "controlBlock") {
            subBlocks++;
            var subElem = addControlBlockToPage(child);
            add_children(table, table_entry("", subElem));
        } else if (child.tagName == "mode"){
            ...
            if (blockMode != "Active") {
                ...
                var options = new Array();
                var max_options = controlModeOptions.length;
                for (var j = 0; j < max_options; j++){
                    var option=document.createElement("option");
                    option.text = controlModeOptions[j];
                    options[j] = option;
                 }
            modeDD = dropdown(blockId + ".mode",options,child.textContent);
        } else if (child.tagName == "id"){
            ...
            parameters = getParametersTable(blockId, "", child);
        }
    }
    // add block parameters
    if (parameters != undefined){
...
// build the form with the audioSystemsTuning argument, so it returns only the audio DSP systems
// and not the voice or streams
    getXMLFromPlayer("../getDSP?audioSystemsTuning", configDSPHelper);
...
function onExportSelectedBlocks()
{
    ...
    // set the preset name to default (since that is what is used by our code)
    var presetName = presetsXML.firstElementChild.getAttribute("name");
    presetsXML.firstElementChild.setAttribute("name", "default");
    presetsXML.firstElementChild.setAttribute("tuningName", presetName);
    ...
    // Remove all the unwanted tags such as Version, Buffers etc.
    var unwantedTags = ["Version", "debug", "inputBuffers", "outputBuffers", "scratchBuffers"];
    unwantedTags.forEach(tag => removeTagFromDoc(sparseXML, tag));
    var xmlserializer = new XMLSerializer();
    var xmlString;
...
function removeTagFromDoc(doc, tag)
{
    var occuranceList = doc.getElementsByTagName(tag);
    if (occuranceList.length > 0) {
        for (var idx = 0; idx < occuranceList.length; idx++) {
            // remove this entry
            occuranceList.item(idx).remove();
        }
    }
}
...
function savePreset(saveToPlayer){
    ...
    if (this.className != "disabled_form_button") {
        ...
        function sendEQCallback(){
            getXMLFromPlayer("../getDSP?audioSystemsTuning", savePresetHelper);
        }
...
function setPersistentEQ(){
    ...
    sendEQDataToZP(sendEQCallback);
    function sendEQCallback(){
        getXMLFromPlayer("../getDSP?audioSystemsTuning", setPersistentEQHelper);
    }
    function setPersistentEQHelper(XMLDoc){
        displayMessage("Saving presets to persistentEQ.xml ...");
        var xmlserializer = new XMLSerializer();
        var xmlString;
        xmlString = xmlserializer.serializeToString(XMLDoc)
        postDataToZP('../setPersistentEQ', xmlString, setPersistentEQCallback);

        function setPersistentEQCallback() {
            stopCurrentlyWorking();
            displayMessage("persistentEQ saved successfully")
        }
    }
}

function removeDSPDebugFiles(){
    if (getCurrentlyWorking()){
        displayMessage(gCurrentlyWorkingMessage);
        return;
    }
    startCurrentlyWorking();
    displayMessage("Removing DSP debug files ...");
    var dummyData = "dummy";
    postDataToZP('../removeDSPDebugFiles', dummyData, removeDSPDebugFilesCallback);

    function removeDSPDebugFilesCallback() {
        stopCurrentlyWorking();
        displayMessage("Persistent DSP debug files removed successfully")
    }
}
...
function findBlockandAddExportTag(root, blockName)
{
    for (var i = 0; i < root.childNodes.length; i++) {
        var childNode = root.childNodes[i];
        if (childNode.nodeName == "block") {
            var curBlockName = childNode.getElementsByTagName("id")[0].childNodes[0];
            if (curBlockName.nodeValue == blockName) {
...
{   // called when the export preset checkbox is toggled
    var blockName = this.id.substring(this.id.indexOf('_') + 1);
    if (this.checked) {
...
function sendEQDataToZP(callback) {
    ...
    for ( var z = 0; z < forms.length; z++ ) {
        if(forms[z].type == "RampIIRBlock" || forms[z].type == "IIRBlock" || forms[z].type == "XFadeIIRBlock" || forms[z].type == "XFadeTwinIIR"){
            block = createIIRBlockXML(xmlDoc, forms[z]);
        }
        else if (forms[z].type == "FIRBlock") {
            block = createFIRBlockXML(xmlDoc, forms[z]);
        }
        else if (forms[z].type == "controlBlock") {
            block = createControlBlockXML(xmlDoc, forms[z]);
        }
        else {
...
function addOtherButtons(showDirty){
    ...
    div("other_buttons", "", [
        ...
        createFormButton("set_persistent_button", "Set Preset as Persistent", setPersistentEQ),
        createFormButton("remove_persistent_button", "Remove Persistent", removeDSPDebugFiles)
...
function clearAndRegeneratePageFromCurrentState(showDirty){
    getXMLFromPlayer("../getDSP?audioSystemsTuning", function(XMLDoc) {clearAndRegeneratePageFromXML(XMLDoc, showDirty)});
}
